...
// ALGORITMA
	for (indeks idx = GetFirstIdxKol(M); idx <= GetLastIdxKol(M); idx++) {
		count += Elmt(M,i,idx);
		jml += 1;
	}
	return (count/jml);
...
// KAMUS LOKAL
	...
	float jml = 0;
// ALGORITMA
	for (indeks idx = GetFirstIdxBrs(M); idx <= GetLastIdxBrs(M); idx++) {
		count += Elmt(M,idx,j);
		jml += 1;
	}
	return (count/jml);
...
// KAMUS LOKAL
	indeks idx;
	int maxBrs = Elmt(M,i,1);
	int minBrs = Elmt(M,i,1);
	
// ALGORITMA
	if (GetLastIdxKol(M)>1) {
		for (idx = GetFirstIdxKol(M)+1; idx <= GetLastIdxKol(M); idx++) {
				if (maxBrs < Elmt(M,i,idx)) {maxBrs = Elmt(M,i,idx);}
				if (minBrs > Elmt(M,i,idx)) {minBrs = Elmt(M,i,idx);}
		}
	}
	*max = maxBrs;
	*min = minBrs;
...
// KAMUS LOKAL
	indeks idx;
	int maxKol = Elmt(M,1,j);
	int minKol = Elmt(M,1,j);
	
// ALGORITMA
	if (GetLastIdxBrs(M)>1) {
		for (idx = GetFirstIdxBrs(M)+1; idx <= GetLastIdxBrs(M); idx++) {
				if (maxKol < Elmt(M,idx,j)) {maxKol = Elmt(M,idx,j);}
				if (minKol > Elmt(M,idx,j)) {minKol = Elmt(M,idx,j);}
		}
	}
	*max = maxKol;
	*min = minKol;
